...
public class EmojiUtil {
    ...
    private static final HashMap<String, Integer> mEmojiMap = new HashMap<String, Integer>();

    // 匹配： [高兴] 的正则表达式
    private static final Pattern PATTERN_EMOJI = Pattern.compile("\\[([A-Za-z\u4E00-\u9FA5]+)\\]");
     ...
     * @param textView
     * @param text      要显示的文本
     * @param emojiSize 表情的大小
     */
    public static void setTextWithEmoji(TextView textView, String text, int emojiSize) {
        Context context = textView.getContext();
        SpannableString ss = new SpannableString(text);

        Matcher m = PATTERN_EMOJI.matcher(ss);
        while (m.find()) {
            String emoji = m.group(); // 匹配到的表情符
            if (mEmojiMap.containsKey(emoji)) { // 过滤掉形如：[xxx] 格式的非表情符
                // System.out.println("-----------emoji: " + emoji);

                int start = m.start(); // 开始位置
                int end = m.end(); //结果位置

                Bitmap bitmap = createFixSizeBitmap(context, mEmojiMap.get(emoji), emojiSize);
    ...
    public static void setTextWithEmoji(TextView textView, String text) {
        setTextWithEmoji(textView, text, dp2px(textView.getContext(), 20));
    }
     ...
     * @param context
     * @param imageResId
     * @param bitmapSize 图片的高度（宽度）
     * @return
     */
    private static Bitmap createFixSizeBitmap(Context context,
                                              int imageResId, int bitmapSize) {
        ...
        float scaleW = (float) bitmapSize / rawWidth; // 宽度的缩放比例
        float scaleH = (float) bitmapSize / rawHeight; // 宽度的缩放比例

        Matrix m = new Matrix();
        m.postScale(scaleW, scaleH); // 设置缩放比例

        return Bitmap.createBitmap(bitmap, 0, 0, rawWidth, rawHeight, m, true); // 设成false图片会有点失真
    }
